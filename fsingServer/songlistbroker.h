...
public:
    ...
    std::vector<std::shared_ptr<SongList>> findSongListsByUserName(std::string username);      //find user's createdSongLists
    ...
    std::vector<std::shared_ptr<SongList>> findSongListsByCollectionRelation(std::string username);
private:
    SongListBroker();
